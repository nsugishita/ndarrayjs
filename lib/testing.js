    ...
    var _assert_deep_equal_impl = function(a, b) {
        ...
        if (Array.isArray(a)) {
            if (a.length != b.length) {
                throw new Error('arrays length mismatch: ' + a.length + ' vs ' + b.length);
            }
            for (var i = 0; i < a.length; i++) {
                try {
                    _assert_deep_equal_impl(a[i], b[i]);
                } catch(e) {
                    msg = e.message;
                    if (msg.startsWith('[')) {
                        e.message = "[" + i + "." + e.message.substr(1);
                        throw e;
                    } else {
        ...
        // Otherwise they're Objects, Functions or some kind of host object
