(function (root, definition) {
    ...
    var buffer_pointer = exported.buffer_pointer = function(array) {
        ...
        var it = internal.iterator_from_shape(array.shape);
        var v = it.next();
        var i = 0;
        var pointer;
        while (!v.done) {
            pointer = internal.dot_jsarray(v.value, array.stride) + array.offset;
    ...
    var accessor_impl = function(a, index, value) {
        ...
        if (is_advanced_indexing && is_getter) {  // advanced getter
            ...
            if (!Array.prototype.some.call(index, isNaN)) {  // single element.
                ...
                it = internal.iterator_from_shape(a.shape);
                v = it.next();
                while (!v.done) {
    ...
    var setitem = exported.setitem = function(a, index, value) {
        ...
        var pointer = internal.dot_jsarray(index, a.stride) + a.offset;
    ...
    exported.apply_advanced_indexing = function(a, index){
        ...
        var axis = internal.arange_jsarray(a.ndim);
        var index_array_like = index.map(array_like);
        var index_array_pos = internal.nonzero_jsarray(index_array_like);
        // Broadcast arrays.
        var broadcasted = np.broadcast_arrays(internal.get_jsarray(index, index_array_like));
        for (i = 0; i < n_advanced_index; i++) {
            index[index_array_pos[i]] = broadcasted[i];
        }
        var original_pointer = exported.buffer_pointer(a);
        axis = internal.get_jsarray(axis, index_array_like).concat(
            internal.get_jsarray(axis, internal.neg_jsarray(index_array_like))
        );
        original_pointer = np.transpose(original_pointer, axis);  // TODO
        index = internal.get_jsarray(index, axis);
        ...
        var it = internal.iterator_from_shape(advanced_index[0].shape);
        var v = it.next();
        var cursor;
        var buf;
        while (!v.done) {
        ...
        if (successive_arrays && (first_array_pos > 0)) {
            ...
            axis = internal.arange_jsarray(new_pointer.ndim);
    ...
    var ascontiguous = exported.ascontiguous = function(a) {
        var default_stride = internal.default_stride(a.shape);
        var i;
        ...
        var it = internal.iterator_from_shape(a.shape);
        var v = it.next();
        i = 0;
        while (!v.done) {
